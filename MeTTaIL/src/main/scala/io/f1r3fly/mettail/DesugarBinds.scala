...
object DesugarBinds {
  def transform(basePres: BasePres): BasePres = {
    BasePresOps.copyPres(
      basePres,
      listdef = Some(addDesugaredLambdas(basePres.listdef_).asScala.toList)
    )
  }
  ...
  def addDesugaredLambdas(listDef: ListDef): ListDef = {
    ...
    for (defn <- listDef.asScala) {
      ...
      // Maps bound variables to their shape
