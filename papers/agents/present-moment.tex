...
\end{align*}
In English: an agent that will run $\code{P}$ after receiving a message $\code{y}$ on the channel $\code{x}$ running concurrently with an agent that is sending the code of $\code{Q}$ on channel $\code{x}$ reduces in a single step to an instance of $\code{P}$ in which every mention of $\code{y}$ has been replaced by the code of $\code{Q}$, written $\code{@Q}$.
...
In English: if $\code{Q}$ reduces in at most $\code{n}$ steps to any of $\code{\{ Q}_1\code{,} \ldots\code{, Q}_m \code{\}}$, then an agent that will run $\code{P}$ after receiving a message $\code{y}$ on the channel $\code{x}$ running concurrently with an agent that is sending the $\code{n}$-step future of the code of $\code{Q}$ (i.e. the states reachable in at most $\code{n}$ steps from $\code{Q}$) on channel $\code{x}$ reduces in a single step to an instance of $\code{P}$ in which every mention of $\code{y}$ has been replaced by the code of the set $\code{\{ Q}_1\code{,} \ldots\code{, Q}_m \code{\}}$, written $\code{@\{ Q}_1\code{,} \dots\code{, Q}_m \code{\}}$.

\medskip

The astute reader will have spotted a potential circularity. In the original rule everything works because the definition of the reduction relation, $\rightarrow$, depends on nothing (apart from the mechanics of substitution and channel equality). In this new rule the definition of the rule seems to depend on the rule! We need to consult how $\code{Q}$ evolves, but surely that will appeal to the consequent of the rule. As with many procedures, processes, and algorithms in life $\code{0}$ comes to our rescue, for $\code{Q} \xrightarrow{\code{0}} \code{Q}!$ So, as long as we have a way to extract $\code{Q}$ from $\code{\{ Q \}}$ the recursion in this formulation grounds out in the original rule.
...
As mentioned above this feature is closely related to the work of Dybvig, Peyton-Jones, and Sabry on a monadic account of delimited continuations \cite{DBLP:journals/jfp/DybvigJS07}. Of course, their work is firmly situated in the context of confluent and sequential rather than concurrent computation (the $\lambda$-lambda calculus), and relies heavily on denotational and categorical techniques. The \code{rho calculus}, by contrast, is aimed at capturing computational phenomena more closely aligned with the real world, where people can race to get the last seat on an airline flight and there is a clear winner and a clear loser, where the competition for resources drives evolution in species, and the competition amongst molecules gives rise to a variety of chemical and biological phenomena. In other words, computational phenomena that do not enjoy confluence.

\medskip

Likewise, operational semantics and rewriting, as opposed to denotational semantics, is emerging as a clear winner not only in implementation of programming languages, but in fields as far ranging as quantum mechanics (see the ZX-Calculus \cite{DBLP:journals/corr/abs-2303-03163}).

\medskip

It should be mentioned that our work on the semantics of the agentic AI language \code{MeTTa} has influenced this work \cite{meredith2023metamettaoperationalsemanticsmetta}. It was a close analysis of how to compile \code{MeTTa} into \code{rholang} efficiently that led to these discoveries. With this small change, \code{rholang} is able to subsume all the features of \code{MeTTa} and provide significantly more. More on this later, too.
...
In \code{rholang} there are builtin primitives for set manipulation as well as pattern-matching for picking apart terms like $\code{Q}_1 \code{|} \ldots \code{|  Q}_m$. Thus, while these design deliberations are of keen interest they are not the focus of the investigation here. We need a mechanism to collect the $n$-step fringe of evolution from $\code{Q}$ and to deliver that collection to the continuation of the for-comprehension such that it can access the elements. In the rest of this paper we will simply assume that apparatus. Thus, for example, we will write expressions like $\code{for(@\{ pattern,} \dots \code{ \} <- x )P}$ and take it to extract all elements from $\code{S}$ that match $\text{pattern}$ when $\code{S}$ is communicated on $\code{x}$, i.e. $\code{x!(S)}$. Readers looking for more detail can see the \code{MeTTaIL} design documentation and the implementation \cite{MeTTaILGithubRepo}.

Lifting our gaze up to take in the whole forest, we see that the $\code{n}$ in the hypothesis of the $\code{spice}$ rule is just what the AI community has been calling the tokenization of inference. As such, this model can slot $\code{LLM}$s directly into agentic computation in the $\code{spice calculus}$. Such systems are simply (stochastic) rewrite systems. $\code{MeTTaIL}$ accounts for agentic computation over entire rewrite systems. More on this later.

\subsubsection{On races and compression}

Note that in either variant, we may express a race condition in one of two forms.

\begin{align*}
  \code{x!( Q}_1 \code{) | for( y <- x )P | x!( Q}_2 \code{)} \\
  \code{for( y <- x )P}_1 \code{| x!( Q ) | for( y <- x )P}_2 \\
\end{align*}

In the first case, when we peer into the future and do not prune the alternatives we must track $\code{P\{ @Q}_1 \code{/ y \}}$ and $\code{P\{ @Q}_2 \code{/ y \}}$. This potentially results in large swaths of P being repeated. In the other case, $\code{Q}$ is repeated. This analysis allows us to see just where copies are necessarily introduced. Whenever $\code{n} > 0$ in a communication and we have race conditions we will see copying. And, the larger $\code{n}$ is and the more races there are, the more reduction will incur copying costs. Fortunately, trie-based compression comes to the rescue! It will per force only record the copies once and differentiate them via path information. This means that implementations of the $\code{spice calculus}$ can perform a syntactic analysis to determine when to introduce trie-based representations of terms \cite{DBLP:journals/corr/abs-2302-08775} and execution and otherwise simply use a hash-based method for finding redexes otherwise.

\subsection{What these means for rho-based agents}

One of the most important developments of the mobile process calculi is to drag the environment of a computation into the computational world. We might understand this in terms of the original motivations of the $\pi$-calculus to address a significant failing of Mealy and Moore machines \cite{enwiki:1325231846}. In those formalisms the environment is not treated on par (pun intended) with the state machine. As a result the model is not compositional. With the parallel composition operator (\code{par}) we can model the environment of an agent as an agent. That is
\[
\code{agent|environment}
\]
Indeed, from the \code{environment}'s perspective, it is the agent and \code{agent} is the environment. That is, if either are capable of entertaining a perspective at all.

\medskip

This symmetry between environment and agent reflects something crucial about the world of computation. \emph{Computations are ontologically isolated.} The only way an agent can fruitfully engage with an environment is if the events generated by the environment have a representation in the model of computation the agent is written in. Think about it this way. For a \code{Java} program to fruitfully engage the events coming from a sensor the events have to have a representation as a \code{Java} data structure. Likewise, for our brains to fruitfully engage the range of electromagnetic events that correspond to what our senses pick up, these events have to be encoded into signals the brain can process.
...
\[
  ...
  where $\mathsf{FN}$ denotes the free names of an agent (see \cite{DBLP:journals/entcs/MeredithR05}), and $\mathsf{N}$ denotes all the names an agent may interact on.
...
\begin{align*}
  \mathsf{PM}_{int}(\text{agent}, \text{environment}) \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad& & \\
  = \{ K( x ) : x \in \mathsf{int}(\text{agent}, \text{environment}), \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\\
...
The past only comes into being if there is some record or trace of the interactions an agent may have with its environment or internally. This is true of the entire calculus as well as of any particular agent. There is a transformation into a particular form for each agent, akin to continuation-passing representation in the $\lambda$-calculus \cite{StracheyWadsworth1974}, that automatically delivers this trace. The form is dubbed \emph{continuation-saturated form} and was originally developed for the \code{rho calculus} to support automatic conversion of computations into reversible computations. The key idea is to intercept every action and record it and all relevant state before taking it. That's what's necessary for reversibility. It turns out to also keep a record of actions taken and any state changes that need to be remembered to put things back the way they were. In other words, agents in \emph{continuation-saturated form} have a past.
...
While this approach to agentic coordination and reasoning is both compact and comprehensive in scope it is not yet as efficient to render to modern hardware as neural nets are. We are exploring different ways to render this model to vector-based computation. In particular we have a translation of \code{GSLTs} to \emph{hypervectors} \cite{DBLP:journals/cogcom/Kanerva09} and hypothesize this reduces the total number of machine instructions relative to our symbolic implementation. Similarly, there is a translation of directed graphs to \emph{upper triangular matrices} that forms the basis of a matrix interpretation of the \code{spice calculus} \cite{DBLP:journals/corr/abs-2301-08181}. None of this work is far enough along yet to confirm or refute whether the model enjoys efficiency gains.

Another important direction is to connect to the physical world the abstraction of computation as channel-based communication. One way to approach this is to show how the usual methods of finding signal in time series data sets work quite well. For example, Fourier and wavelet transforms result in natural channel sets. In the first case frequency bands where signals are found are the channels. This is approach is so natural as to be almost uninteresting. What is considerably more interesting is to co-evolve the channel sets (aka namespaces) connecting agents to an environment through a dialogue between the agent and the environment. This dynamic reconfiguration of agentic behavior as the namespace changes is the whole \emph{raison d'etre} of the rho family of calculi.

It should not go without remark that this conversion of time series data into channel-based communication is exactly what happens for brains and computer hardware. Neurons and transistors are all about the conversion of the continuous domain of electromagnetic phenomena into channel-based communication.

\bibliographystyle{plain}   
\bibliography{present-moment.bib}
