...
Recapping, the \code{rho calculus} comprises the following syntactic categories of processes.
\begin{itemize}
    \item \code{0}
    \item \code{x!( P )}
    \item \code{for( y <- x )P}
...
\begin{align}
  \msgrecv{x}{y}{P} \parcomp \msgsend{x}{Q} &\rightarrow P\{ @Q / y \} \quad (\tcode{COMM}) \\
  P \rightarrow P' &\implies P\parcomp Q \rightarrow P'\parcomp Q \quad (\tcode{PAR}) \\
  P = P', P' \rightarrow Q', Q' = Q &\implies P \rightarrow Q \quad (\tcode{STRUCT})
\end{align}
...
In what follows it will be convenient to write $P\downarrow_{x}$ just when $P = for( y \leftarrow x )P \parcomp x!( Q ) \parcomp \ldots$.

Likewise, it will be convenient to define the collection of contexts for the rho calculus recursively as

$K ::= \square \mid \msgrecv{x}{y}{K} \mid \msgsend{x}{K} \mid P\parcomp K$

We use contexts to characterize and label interactions. For example, when $K = \square \parcomp \msgsend{x}{Q}$ we say $\msgrecv{x}{y}{P}$ transitions via $K$ to $P\{ @Q / y \}$, written $\msgrecv{x}{y}{P} \xrightarrow{K} P\{ @Q / y \}$. Conversely, when $K = \msgrecv{x}{y}{P} \parcomp \square$ we have $\msgsend{x}{Q} \xrightarrow{K} P\{ @Q / y \}$. This is merely a way of repackaging the $\code{COMM}$ rule to get a notion of labelled transition which delivers a bisimulation which is a congruence. See \cite{Wells2025} for more details.

This means we can abuse notation and write $P\downarrow_{K}$ without ambiguity. Note that when we wish to emphasize or call out the name of a context that labels a transition, we write $K(x)$, and hence $P\downarrow_{K(x)}$.
...
\begin{align*}
  P, Q &\coloneqq 0 \mid \msgrecv{x}{y}{P} \mid \msgsend{x}{Q} \mid P\parcomp Q \mid *x \mid \{ P* \} \\
  x, y &\coloneqq @P
\end{align*}
...
\begin{equation}
  Q \xrightarrow{n} \{ Q_1, \dots, Q_m \} \implies \msgrecv{x}{y}{P} \parcomp \msgsend{x}{Q}\precog{n} \rightarrow P\{ @\{ Q_1 \parcomp \dots \parcomp Q_m \} / y \} \quad (\tcode{LOOKAHEAD})
\end{equation}

In \code{rholang} there are builtin primitives for set manipulation as well as pattern-matching for picking apart terms like $Q_1 \parcomp \dots \parcomp Q_m$. Thus, while these design deliberations are of keen interest they are not the focus of the investigation here. We need a mechanism to collect the $n$-step fringe of evolution from $Q$ and to deliver that collection to the continuation of the for-comprehension such that it can access the elements. In the rest of this paper we will simply assume that apparatus. Thus, for example, we will write expressions like $\msgrecv{x}{@\{ \text{pattern}, \dots \}}{P}$ and take it to extract all elements from $S$ that match $\text{pattern}$ when $S$ is communicated on $x$, i.e. $\msgsend{x}{S}$. Readers looking for more detail can see the \code{MeTTaIL} design documentation and the implementation.
